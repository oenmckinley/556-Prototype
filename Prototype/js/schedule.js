...
function update_schedule(dayone){
    ...
    hover_div.getElementsByClassName('meal_hover_favorite')[0].addEventListener('click',function(){
        const url = `4.profile.html`;
        window.location.href = url;
    });
                ...
                if((!to_breakfast)&&(!to_lunch)&&(!to_dinner)){
                    ...
                    while(date_from<=date_to){
                        ...
                        if(to_breakfast){
                            ...
                            scheduled_meals_data[key]['breakfast'] = scheduled_meals_data[key]['breakfast'].concat(name_to_add);
                        }
                        if(to_lunch){
                            if(!('lunch' in scheduled_meals_data[key])){
                                scheduled_meals_data[key]['lunch'] = [];
                            }
                            scheduled_meals_data[key]['lunch'] = scheduled_meals_data[key]['lunch'].concat(name_to_add);
                        }
                        if(to_dinner){
                            if(!('dinner' in scheduled_meals_data[key])){
                                scheduled_meals_data[key]['dinner'] = [];
                            }
                            scheduled_meals_data[key]['dinner'] = scheduled_meals_data[key]['dinner'].concat(name_to_add);
                        }
